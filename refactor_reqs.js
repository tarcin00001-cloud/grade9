...
const original = fs.readFileSync('labs/RequirementAnalysis9.tsx', 'utf8');
const lines = original.split('\n');

const logicPart = lines.slice(0, 359).join('\n');
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mt-auto pt-1.5 border-t border-slate-800">
                        <span className={\`text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded \${
                           mod.type === "solution" || mod.type === "vague" ? "bg-slate-800 text-slate-500" : "bg-slate-800 text-slate-500" 
                        }\`}>
                ...
                {simStatus === "testing" ? (
                  ...
                  <div className="flex flex-col items-center gap-3 w-full">
                    <div className="grid grid-cols-2 gap-2 w-full max-w-[200px]">
                      {slottedF.map((id) => (
                        <div key={id} className="p-1.5 rounded-lg bg-cyan-950/40 border border-cyan-500/30 text-[9px] font-bold text-cyan-300 flex items-center justify-center gap-1.5">
                          {renderIcon(ALL_MODULES[id].iconName, 12)}
                        </div>
                      ))}
                      {slottedNF.map((id) => (
                        <div key={id} className="p-1.5 rounded-lg bg-emerald-950/40 border border-emerald-500/30 text-[9px] font-bold text-emerald-300 flex items-center justify-center gap-1.5">
                          {renderIcon(ALL_MODULES[id].iconName, 12)}
                        </div>
                      ))}
                    </div>
...
const finalFile = logicPart + '\n' + newUI;
fs.writeFileSync('labs/RequirementAnalysis9.tsx', finalFile);
console.log("Success");
