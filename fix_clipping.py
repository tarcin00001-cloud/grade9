      ...
      <div className="w-full flex-1 flex flex-col gap-3 md:gap-4 relative isolate pb-2 max-w-7xl mx-auto min-h-0">
        ...
        <div className={`bg-white rounded-2xl border p-3 md:p-4 shrink-0 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm transition-colors ${stats.cpuTemp > 80 ? 'border-rose-300 bg-rose-50' : 'border-slate-200'}`}>
            <div className="flex items-center gap-4 w-full md:w-auto">
                <div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center shrink-0 border ${stats.cpuTemp > 80 ? 'bg-white border-rose-300 text-rose-600 animate-pulse' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
                ...
                <div className="flex-1">
                    ...
                    <p className={`text-[10px] md:text-xs font-semibold leading-tight ${stats.cpuTemp > 80 ? 'text-rose-600' : 'text-slate-500'}`}>
            ...
            <div className="flex gap-4 md:gap-6 relative z-10 shrink-0 bg-slate-50 border border-slate-200 rounded-xl px-4 md:px-6 py-2 shadow-inner">
               <div className="text-center flex flex-col justify-center">
                  <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest">RAM Hits</div>
                  <div className="text-xl md:text-2xl font-black text-emerald-600 leading-none mt-1">{stats.hits}</div>
               </div>
               <div className="w-px bg-slate-200" />
               <div className="text-center flex flex-col justify-center">
                  <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Page Faults</div>
                  <div className="text-xl md:text-2xl font-black text-amber-500 leading-none mt-1">{stats.faults}</div>
               </div>
               <div className="w-px bg-slate-200" />
               <div className="text-center flex flex-col justify-center">
                  <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest">CPU Temp</div>
                  <div className={`text-xl md:text-2xl font-black leading-none mt-1 ${stats.cpuTemp > 80 ? 'text-rose-600 animate-pulse' : 'text-slate-700'}`}>{stats.cpuTemp}°C</div>
               </div>
            </div>
        </div>
        ...
        {phase === "intro" && (
          <div className="absolute inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-slate-900/20 rounded-3xl">
             <div className="max-w-md w-full bg-white border border-slate-200 rounded-3xl p-6 md:p-8 shadow-2xl text-center">
                ...
                <h1 className="text-xl md:text-2xl font-black text-slate-800 uppercase tracking-widest mb-4">Virtual Memory System</h1>
        ...
        {phase === "thrashing_intro" && (
          <div className="absolute inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-slate-900/20 rounded-3xl">
             <div className="max-w-md w-full bg-white border border-amber-200 rounded-3xl p-6 md:p-8 shadow-2xl text-center">
                <AlertTriangle size={48} className="text-amber-500 mb-6 mx-auto" />
                <h1 className="text-xl md:text-2xl font-black text-slate-800 uppercase tracking-widest mb-4">Demand Spike Detected</h1>
                <p className="text-slate-500 font-medium text-xs md:text-sm leading-relaxed mb-8">
        ...
        {phase === "upgrade_ssd" && (
          <div className="absolute inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-md bg-rose-950/60 rounded-3xl">
             <div className="max-w-md w-full bg-white border border-rose-200 rounded-3xl p-6 md:p-8 shadow-[0_0_60px_rgba(244,63,94,0.3)] text-center animate-in zoom-in duration-300">
                ...
                <h1 className="text-xl md:text-2xl font-black text-rose-600 uppercase tracking-widest mb-4">THRASHING DETECTED</h1>
                <p className="text-slate-600 font-medium text-xs md:text-sm leading-relaxed mb-8">
        ...
        {/* 100% FLUID HEIGHT CHASSIS */}
        <div className={`w-full flex-1 flex flex-col md:flex-row bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden min-h-0 transition-all ${phase === 'thrashing' && stats.cpuTemp > 85 ? 'animate-shake pointer-events-none border-rose-400 shadow-[0_0_50px_rgba(244,63,94,0.2)]' : ''}`}>
           
           {/* Diagnostics Column (The Control Panel) */}
           <div className="flex flex-col shrink-0 w-full md:w-64 lg:w-72 bg-slate-50 border-b md:border-b-0 md:border-r border-slate-200 p-3 md:p-5 relative z-10" style={{ maxWidth: "100%" }}>
              <h3 className="text-xs font-black text-slate-700 uppercase tracking-widest mb-2 md:mb-4 flex items-center gap-2 relative">
                 <Activity size={16} className="text-cyan-600" /> Diagnostics
              </h3>
              
              <div className="shrink-0 h-20 md:h-24 rounded-xl bg-slate-900 border-[3px] border-slate-200 shadow-[inset_0_4px_15px_rgba(0,0,0,0.8)] flex flex-col items-center justify-center p-3 md:p-4 relative overflow-hidden font-mono mt-1 mb-4 md:mb-6">
                 ...
                 {activeDemand ? (() => {
                    ...
                    return (
                       <div className="flex flex-col items-center gap-1 animate-in fade-in zoom-in text-center relative z-10">
                          <span className="font-bold uppercase tracking-widest text-[11px] md:text-base text-emerald-400 drop-shadow-md truncate w-full max-w-[180px]">{app.name}</span>
                          <span className="text-[9px] md:text-[10px] font-bold text-emerald-300 uppercase tracking-widest animate-pulse mt-0.5">&gt; REQ_RAM</span>
                       </div>
                    );
                 })() : (
                    <span className="text-emerald-500/80 font-black uppercase tracking-widest text-xs md:text-sm drop-shadow-md animate-pulse">NO SIGNAL</span>
                 )}
              </div>

              <h3 className="text-xs font-black text-slate-700 uppercase tracking-widest mb-2 md:mb-3 relative flex items-center justify-between">
                 Data Bus Queue
              </h3>
              
              <div className="flex flex-row flex-wrap gap-2 md:gap-3 relative overflow-y-auto">
                 <AnimatePresence>
                    {queue.map(appId => {
                       ...
                       return (
                          <motion.div
                            ...
                            className={`bg-white border border-slate-300 border-b-[3px] shadow-sm rounded-lg p-2 md:p-2.5 pr-2 md:pr-3 flex items-center gap-2 md:gap-3 cursor-pointer hover:bg-slate-50 active:border-b-0 active:translate-y-[2px] transition-all flex-1 min-w-[120px] z-20`}
                          >
                 ...
                 {queue.length === 0 && !activeDemand && (
                    <div className="h-10 md:h-12 w-full flex items-center justify-center text-slate-400 font-bold text-xs uppercase tracking-widest">
                       Queue Empty
                    </div>
                 )}
              </div>
           ...
           {/* Motherboard Area - 100% Fluid Flex Grid */}
           <div className="flex-1 flex flex-col min-h-0 relative bg-white">
              
              {/* RAM Section - Flex to fill half the motherboard space */}
              ...
              {/* Disk Section - Flex to fill bottom half */}
                    ...
                    <AnimatePresence>
                       {disk.map(appId => {
                          ...
                          return (
                             ...
                             >
                                <div className={`w-6 h-6 md:w-8 md:h-8 rounded-md ${app.bgColor} flex items-center justify-center border ${app.borderColor} mb-1 md:mb-2`}>
                                   <app.icon size={14} className={app.color} />
                                </div>
...
print(f"Successfully fixed all overlaying clipping in {target_file}.")
