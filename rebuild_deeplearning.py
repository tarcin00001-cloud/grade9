...
export default function DeepLearning9() {
  ...
  const handleTrain = () => {
    ...
    if (willWin) {
      setPhase("training_success");
      // Simulate loss dropping drastically
      let step = 0;
      const history = [0.89];
      const interval = setInterval(() => {
        step++;
        const newLoss = Math.max(0.02, 0.89 - (step * 0.15) - (Math.random() * 0.05));
        history.push(newLoss);
      ...
      setPhase("training_fail");
      // Simulate loss barely moving
      let step = 0;
      const history = [0.89];
      const interval = setInterval(() => {
        step++;
        const newLoss = Math.max(0.75, 0.89 - (step * 0.02));
        history.push(newLoss);
        setLossHistory([...history]);
        setCurrentLoss(newLoss);
        ...
        <div className="h-48 md:h-56 shrink-0 grid grid-cols-1 lg:grid-cols-3 gap-4">
           ...
           <div className="bg-white rounded-3xl border border-slate-200 shadow-md flex flex-col overflow-hidden relative">
              <div className="bg-slate-50 border-b border-slate-200 px-4 py-2 flex justify-between items-center shrink-0">
                 <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Output Panel</span>
              ...
              <AnimatePresence>
                 {phase === "feedback" && (
                    ...
                    >
                       ...
                       <button onClick={() => setPhase("improving")} className="mt-3 px-4 py-1.5 bg-slate-900 text-white text-[10px] uppercase tracking-widest font-bold rounded-full">Got it</button>
                    </motion.div>
                 )}
              </AnimatePresence>
           </div>
...
print("Deep Learning Lab completely rebuilt with Premium Light Theme and 7-Step Flow!")
