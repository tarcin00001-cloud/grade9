...
const playTone = (freq: number, type: OscillatorType, duration: number, delay = 0) => { setTimeout(() => { const ctx = getCtx(); if (!ctx) return; try { const osc = ctx.createOscillator(); const gain = ctx.createGain(); osc.type = type; osc.frequency.setValueAtTime(freq, ctx.currentTime); gain.gain.setValueAtTime(0.12, ctx.currentTime); gain.gain.exponentialRampToValueAtTime(0.00001, ctx.currentTime + duration); osc.connect(gain); gain.connect(ctx.destination); osc.start(); osc.stop(ctx.currentTime + duration); } catch { /* */ } }, delay); };
...
export function useLabAudio() {
  ...
  return { playClick, playPop, playSuccess, playError, playZap, playDrop, playChime, speakInstructions, stopSpeaking };
}
