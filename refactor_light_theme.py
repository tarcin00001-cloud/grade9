...
replacements_colors = [
    ...
    # Note: text-slate-500 is fine as is
    ...
    ('bg-slate-800', 'bg-slate-200'), # tags on cards
    
    # Phone Simulator Background (Keep it dark, but the shell is light)
    # The phone screen is bg-slate-100 right now because we replaced 950 with 100. Let's fix that back to dark for the screen.
...
# Phone text needs to be white since the screen is dark
content = content.replace('text-slate-800 uppercase tracking-[0.15em] mb-4">Live Simulator', 'text-slate-800 uppercase tracking-[0.15em] mb-4">Live Simulator') # Outside screen
content = content.replace('className="text-[10px] text-slate-500 font-mono"', 'className="text-[10px] text-slate-400 font-mono"') # inside screen top bar
...
# The button in Simulator was changed from text-white to text-slate-900. Revert it.
...
# The text inside the right pane "Next Stage" button
...
# Update the empty slots text colors (was cyan-500/40, make it darker)
