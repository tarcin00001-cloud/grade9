...
export default function TestingStrategies43() {
  ...
  const labCurrentStep = qaStateToStep(qaState);
                              ...
                              <AnimatePresence>
                                  {isSubmitted ? (
                                      ...
                                      >
                                          ...
                                          {assessmentAns === 2 && (
                                              <button 
                                                  onClick={() => { setQaState('completed'); setTimeout(() => reportComplete({ points: marksForStep(labCurrentStep) }), 4500); }} 
      ...
      {timedOut && !isLabComplete && (
        <div className="absolute inset-0 z-40 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm rounded-2xl">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-6 max-w-sm text-center mx-4">
            ...
            <button onClick={handleLaunchClick} className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 active:translate-y-1 shadow-[0_4px_0_rgba(79,70,229,1)] active:shadow-none text-white rounded-xl text-sm font-bold transition-all cursor-pointer">
              Try Again
            </button>
          </div>
        </div>
