...
import { 
  Activity, ShieldAlert, Cpu, Eye, CheckCircle2, Crosshair, 
  ArrowRight, MousePointer2, Settings2, Stethoscope, AlertTriangle
} from "lucide-react";
...
export default function RoboticSurgery46() {
  ...
  useAnimationFrame(() => {
    ...
    if (!robotActive) {
      // High-frequency, aggressive mechanical noise that forces brutal damage offsets
      const time = Date.now();
    ...
    >
      ...
      <div className="flex-1 w-full flex flex-col min-h-0 max-w-6xl mx-auto p-2 lg:p-4 gap-4">
        ...
        <div className="flex-1 flex flex-col lg:flex-row gap-4 min-h-0 w-full">
          ...
          <div className="flex-1 flex flex-col bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden min-h-0">
             
             <div className="p-3 lg:p-4 border-b border-slate-100 bg-slate-50 shrink-0 flex items-center justify-between">
               <div>
                 <h3 className="text-lg font-bold text-[#123B5D] flex items-center gap-2">
                   <Stethoscope className="text-[#2563EB]" size={20} />
                   {level.title}
                 </h3>
                 <p className="text-sm text-slate-600 mt-1">{level.desc}</p>
               </div>
             </div>
                ...
                {bounds.w > 0 && (
                  <>
                    <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-90 transition-all duration-700">
                       <rect width="100%" height="100%" fill="#2a0a18" />
                    ...
                    >
                       ...
                       {/* The physical scalpel ring with violent high-frequency linear shake */}
                       <motion.div 
          ...
          <div className="w-full lg:w-80 flex flex-col gap-4 shrink-0 min-h-0">
             ...
             <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
                <h3 className="text-sm font-bold text-[#123B5D] flex items-center gap-2 mb-4 uppercase tracking-wider">
                  <Activity size={16} className="text-[#E11D48]" /> Patient Telemetry
                </h3>
                
                <div className="mb-4">
                  <div className="flex justify-between text-xs font-bold mb-1.5">
                    <span className="text-slate-600">Tissue Trauma</span>
                    <span className={damage > 70 ? 'text-[#E11D48]' : damage > 40 ? 'text-[#F59E0B]' : 'text-[#10B981]'}>
                      {Math.floor(damage)}%
                    </span>
                  </div>
                  <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden border border-slate-200">
                    <div 
                      className={`h-full transition-all duration-100 ${damage > 70 ? 'bg-[#E11D48]' : damage > 40 ? 'bg-[#F59E0B]' : 'bg-[#10B981]'}`}
                      style={{ width: `${damage}%` }}
                    />
                  </div>
                </div>
                ...
                <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                  <div className="text-[10px] text-slate-500 font-bold mb-2 uppercase tracking-widest">System Status</div>
                  <div className="text-xs font-bold space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="text-slate-600">Tremor Status:</span>
                      <span className={`px-2 py-0.5 rounded ${robotActive ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                        {robotActive ? 'ELIMINATED' : 'DETECTED'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-slate-600">Visual Feed:</span>
                      <span className={`px-2 py-0.5 rounded ${visionEnhanced ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                        {visionEnhanced ? '3D MAGNIFIED' : 'STANDARD 2D'}
                      </span>
                    </div>
                  </div>
                </div>
             ...
             <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex-1 lg:flex-none">
                <h3 className="text-sm font-bold text-[#123B5D] flex items-center gap-2 mb-4 uppercase tracking-wider">
                  <Settings2 size={16} className="text-[#2563EB]" /> Surgical Controls
                </h3>
                
                <div className="space-y-3">
                  <div className={`p-3 rounded-xl border transition-all ${
                    !level.robotAllowed ? 'opacity-40 grayscale bg-slate-50 border-slate-100' :
                    robotActive ? 'bg-[#F0FDF4] border-[#10B981]/30 shadow-sm' : 
                    isRobotHint ? 'bg-amber-50 border-amber-300 shadow-[0_0_15px_rgba(245,158,11,0.2)]' :
                    'bg-white border-slate-200 hover:border-[#2563EB]/50'
                  }`}>
                    <div className="flex justify-between items-center mb-1">
                      <span className={`font-bold text-xs flex items-center gap-1.5 ${robotActive ? 'text-[#10B981]' : isRobotHint ? 'text-amber-700' : 'text-slate-700'}`}>
                        <Cpu size={14} className={isRobotHint ? "animate-pulse" : ""} /> Robotic Assistance
                      </span>
                      {level.robotAllowed && (
                        <button 
                          onClick={() => {
                          ...
                          className={`w-10 h-5 rounded-full p-0.5 transition-colors ${robotActive ? 'bg-[#10B981]' : isRobotHint ? 'bg-amber-400' : 'bg-slate-300'}`}
                        >
                    ...
                    <p className="text-[10px] text-slate-500 leading-tight">
                      Eliminates micro-tremors, translating hand movements flawlessly.
                    </p>
                  </div>

                  <div className={`p-3 rounded-xl border transition-all ${
                    !level.visionAllowed ? 'opacity-40 grayscale bg-slate-50 border-slate-100' :
                    visionEnhanced ? 'bg-[#F0FDF4] border-[#10B981]/30 shadow-sm' : 'bg-white border-slate-200 hover:border-[#2563EB]/50'
                  }`}>
                    <div className="flex justify-between items-center mb-1">
                      <span className={`font-bold text-xs flex items-center gap-1.5 ${visionEnhanced ? 'text-[#10B981]' : 'text-slate-700'}`}>
                        <Eye size={14} /> 3D Enhanced Vision
                      </span>
                      {level.visionAllowed && (
                        <button 
                          onClick={() => {
                          ...
                          className={`w-10 h-5 rounded-full p-0.5 transition-colors ${visionEnhanced ? 'bg-[#10B981]' : 'bg-slate-300'}`}
                        >
                    ...
                    <p className="text-[10px] text-slate-500 leading-tight">
                      Magnifies the surgical field, expanding the safe precision pathway.
                    </p>
                  </div>
                </div>
             </div>
          ...
          {modalState !== 'none' && !isCompleted && (
            ...
            >
              ...
              >
                ...
                {modalState === 'quiz' && (
                  <>
                    <div className="bg-blue-100 p-4 rounded-full text-blue-600 mb-4"><Activity size={32} /></div>
